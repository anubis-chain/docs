---
title: Full Node - Anubis Develop
---
...
### 2.2.Steps
There are 2 approaches to setup a Anubis full node from scratch:

- By Snapshot(Recommend): download the latest snapshot and sync based on it.

- From Genesis(Not Recommend): sync the whole Anubis chain from genesis block.

!!! tip
    As of Nov-2024, the latest block height of Anubis mainnet is over 40M, it would need a more powerful hardware and take a great of time to sync from genesis, so it is suggested to setup a Anubis full node based the snapshot.
...
Please refer to [Anubis-Deploy Tools](https://github.com/anubis-chain/node-deploy) to setup a local private network.
