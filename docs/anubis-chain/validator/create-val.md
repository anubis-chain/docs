---
title: Create Anubis Validator - Anubis Chain
---

# Create Anubis Validator

This guide outlines the process for creating a new validator on the Anubis Chain (Anubis).
The DAI staking dApp is the official tool for creating and managing validators on the Anubis.

- **Mainnet**: [https://anubischain.ai/en/bnb-staking](https://anubischain.ai/en/bnb-staking)

## Terminology

- **Operator Address**: The address for creating and modifying validator information on the Anubis. You should use this
  address when connecting to the staking dApp. The corresponding account should have more than 2001 DAI for creating
...
options are available for the step. Make sure that the account has more than 2001 DAI before moving on to the next step.
...
the [Anubis validator directory](https://github.com/anubis-chain/bsc-validator-directory). Your avatar, once uploaded, will be
...
Download the Anubis geth binary from [the official release page](https://github.com/anubis-chain/bsc/releases/).
...
```shell
geth bls account generate-proof --datadir ${DATA_DIR} --chain-id ${ANUBIS_CHAIN_ID} ${OPEATOR_ADDRESS} ${VOTE_ADDRESS}
```

- `ANUBIS_CHAIN_ID`: `56` for Anubis mainnet, and `97` for Anubis testnet.
...
Identity is an optional field and should be left empty.
...
- **Self Delegate Amount**: The amnout to delegate when creating the validator. The minimal number to input is `2001` -
  for the minimal self delegation amount is 2000 DAI and extra 1 DAI for locking to a dead address.

### 3. Submitting the form

Once you have filled out all the required information, click the `Submit` button to submit the transaction.

Note: Upon completing these steps, your node is not guaranteed to become an active validator. Selection is based on a ranking that reflects the total DAI staked, with only the top N nodes being chosen as active validators. The number N is determined by the "maxElectedValidators" parameter within the StakeHubContract (0x0000000000000000000000000000000000002002). As of November 4th, 2024, this number stands at 8 for the testnet and 45 for the mainnet.
