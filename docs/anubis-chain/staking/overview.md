---
title: Overview - Anubis Staking
---

# Anubis Staking Overview

![Staking](../img/Staking.png)

Anubis Chain (Anubis) operates on a Proof-of-Staked-Authority (PoSA) blockchain, with the staking mechanism proposed in [BEP-294](https://github.com/anubis-chain/BEPs/pull/294).
This enables DAI holders to stake their tokens with specified validators to secure the network and earn staking rewards.
Here's an overview covering the core staking concepts and operations on Anubis.

## Basic Concepts

### Consensus Engine

Anubis uses a consensus mechanism which combines DPoS and PoA for consensus, in this system:
...
Anubis. The validator set is determined by the amount of staking each validator has, which reflects the amount of
DAI staked by the validator and its delegators. The top validators with the most staking are selected as the
...
Similarly, they can opt-out by simply withdrawing all their DAI delegations.
...
After UTC 00:00, the consensus engine sorts validators and updates the Anubis validator set with the ranking information.

### System Contracts

There are several built-in contracts (i.e., system contracts) to facilitate the Anubis staking.
...
between credit and DAI. The token name of a staking credit is "stake {{validator moniker}} credit",
and the symbol is "st{{validator moniker}}". The contract will be created by the Stake Hub Contract when a validator
is created.

Whenever a user delegates DAI, an equivalent quantity of credit tokens are created. On the other hand,
when a user withdraws their delegation, a corresponding amount of credit tokens are destroyed, thereby releasing the DAI.
...
Validators are nodes running Anubis Chain software, participating in the consensus process.
They require a minimum DAI stake at their validator address and can receive delegations from other DAI holders.
Validators earn rewards from transaction fees and share most of these rewards with their delegators.

### Create Validator

To ensure the security of the network, becoming a validator on the Anubis requires a minimum self-delegation of 2000 DAI.
DAI holders can initiate a `CreateValidator` transaction with the `StakeHub` contract to become a validator.
For more information, refer to [Create Anubis Validator](../validator/create-val.md).
...
Delegators are DAI holders who stake their DAI with a validator, sharing rewards.
They can select any active or standby validator, switch between them, undelegate their DAI, and claim rewards anytime.
