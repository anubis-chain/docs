---
title: Build Anubis Staking dApps Guide - Anubis Staking
---

# Build Anubis Staking dApps Guide
...
The Anubis staking mainly uses the smart contracts `StakeHub` for validator and delegation management.
...
> **Note**: Creating a validator requires locking 1 DAI, and the transaction must be sent with a sufficient DAI amount to
cover this lock amount plus any self-delegation, in total 2001 DAI.
...
To delegate DAI to a validator, call the `delegate` function with the following parameters:
...
To undelegate DAI from a validator, use the `undelegate` function with the following parameters:
...
To redelegate DAI from one validator to another, use the `redelegate` function with the following parameters:
...
To claim undelegated DAI after the unbonding period, use the `claim` function for a single request or `claimBatch` for
multiple requests:
...
## Precision Loss
During the conversion process between credit tokens and DAI, it is inevitably encounter the usage of integer division, 
which may results in a precision loss. It can lead to tangible issues. For example, a user who delegates 1 DAI and then 
decides to undelegate immediately. Due to the aforementioned precision loss, they will only be able to claim back 0.99..99 DAI, 
which is essentially 1 minus a tiny fraction (1e-18) of DAI. 
...
Or instead of undelegating, users can exchange their credit tokens for DAI, with the exact conversion results prominently displayed.
...
The credit contract is a ERC20 contract, and the ABI is the same
as [Stake Credit contract](https://github.com/anubis-chain/bsc-genesis-contract/blob/master/abi/stakecredit.abi).

It provides functions for querying delegations, including:

* `balanceOf(address)`: Get the credit balance of a delegator.
* `getPooledAGT(address)`: Get the pooled DAI amount of a delegator.
* `getPooledAGTByShares(uint256)`: Get the pooled DAI amount for a specific amount of shares.
* `getSharesByPooledAGT(uint256)`: Get the shares for a specific amount of pooled DAI.
...
* `lockedAGTs(address, uint256)`: Get the locked DAI for a delegator's unbond queue.

### How to get the shares/DAI for a delegator?

For any specific validator, please call the `balanceOf` function of the validator's creat contract to get the
delegator's shares. To get the DAI amount instead of shares, the function `getPooledAGT` can be used.
...
### How to calculate the DAI amount for a specific amount of shares?

The credit contract provides the `getPooledAGTByShares` function to calculate the DAI amount for some specific amount of
shares.

To do the vice visa, please use the `getSharesByPooledAGT` function to calculate the shares for some
specific DAI amount.
...
The reward is distributed to each validator's DAI pool at 00:00:00 UTC time every day. To calculate the APR/APY of a
validator, the total pooled DAI amount and the propagandising reward amount for the same day are needed.

The `StakeHub` contract provides the `getValidatorTotalPooledAGTRecord(address,uint256)(uint256)`
...
// get the total pooled DAI amount and the crrospanding reward amount for the given validator and index
totalPooledAGT, _ := stakeHub.GetValidatorTotalPooledAGTRecord(nil, validatorOperatorAddress, index)
reward, _ := stakeHub.GetValidatorRewardRecord(nil, validatorOperatorAddress, index)

// calculate the APY
rate, _ := big.NewFloat(0).Quo(big.NewFloat(0).SetInt(reward), big.NewFloat(0).SetInt(totalPooledAGT)).Float64()
...
To get the locked DAI for unbonding requests, please use the `lockedAGTs` function. It has the parameter `number` to
define the sum of first `number` unbonding requests' DAI locked in the delegator's unbond queue. Set the `number`
to `0` to get all the locked DAI.
...
the `getPooledAGT` of the credit contract of a validator, 3) do the math. You can refer 
to [this code repository](https://github.com/anubis-chain/staking-reward-example) for example.
